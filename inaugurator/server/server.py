...
class Server(threading.Thread):
    def __init__(self, checkInCallback, doneCallback, progressCallback, failedCallback):
    ...
    def _listenOnID(self, id):
        ...
        self._channel.exchange_declare(onLabelExchangeDeclared,
                                       exchange_type='fanout', exchange=labelExchange)
    ...
    def _handleStatus(self, channel, method, properties, body):
        try:
            ...
            elif message[u'status'] == "failed":
                self._failedCallback(id, message["message"])
            else:
